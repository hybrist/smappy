...
```typescript
if (chunk.map) {
  const sourceMap = typeof chunk.map === 'string' ? chunk.map : JSON.stringify(chunk.map);
}
```
...
Since Vite uses Rollup, the Vite adapter (`src/lib/server/plugins/vite/adapter.ts`) serves as a reference implementation for Rollup plugins.
