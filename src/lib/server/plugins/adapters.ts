...
export abstract class BundlerAdapter {
  ...
  constructor(baseDir: string, options: BundlerPluginOptions, config?: PluginConfig) {
  ...
  protected convertModules(bundlerModules: BundlerModule[], errors: string[]): ModuleInput[] {
    ...
    for (const bundlerModule of bundlerModules) {
      ...
      if (!this.options.analyzeThirdParty && bundlerModule.identifier.includes('node_modules')) {
        continue;
      }
    ...
    for (const bundlerBundle of bundlerBundles) {
      ...
      if (this.options.extractSourceMaps !== false) {
        if (bundlerBundle.sourceMap) {
          ...
          const extracted = extractSourceMap(bundleContent, bundlePath, this.options.outputDir);
   ...
   */
  protected createIngestionOptions(bundlerType: IngestionOptions['bundlerType']): IngestionOptions {
    return {
      bundlerType,
   ...
   */
  protected resolveDependencies(bundlerModules: BundlerModule[]): Map<string, string[]> {
