...
export async function angularBundleAnalysisBuilder(
  ...
  const { autoIngest = true, buildOutputDir, useStatsJson = true, ...pluginOptions } = options;
  ...
  try {
    ...
    if (!existsSync(outputPath)) {
      logger.error(`[Angular Bundle Analysis] Output directory not found: ${outputPath}`);
    ...
    if (useStatsJson) {
      ...
      if (existsSync(statsPath)) {
        ...
        logger.warn('[Angular Bundle Analysis] stats.json not found, using output directory scan');
      }
    }
    ...
    if (extractionResult.warnings.length > 0) {
      ...
      extractionResult.warnings.forEach((warning) => logger.warn(`  - ${warning}`));
    }
