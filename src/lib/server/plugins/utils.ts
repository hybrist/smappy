...
export function extractSourceMap(
  ...
  if (inlineSourceMapMatch) {
    try {
      const decoded = Buffer.from(inlineSourceMapMatch[1], 'base64').toString('utf-8');
  ...
  const externalSourceMapMatch = bundleContent.match(/\/\/# sourceMappingURL=([^\s]+)/);
 ...
 * @returns Source map content or undefined if not found
 */
export function extractSourceMapFromPath(bundlePath: string): string | undefined {
 ...
 */
export function detectFileType(filePath: string): BundleInput['type'] | ModuleInput['fileType'] {
 ...
 */
export function resolveModulePath(importPath: string, fromPath: string, baseDir: string): string {
  // Absolute path
 ...
 * @returns Size in bytes
 */
export function getSize(size: number | undefined, fallbackContent?: string): number {
  if (size !== undefined) {
    return size;
  }
 ...
 * @returns File content or undefined if file cannot be read
 */
export function readFileContentSafe(filePath: string, errors: string[]): string | undefined {
  try {
    return readFileSync(filePath, 'utf-8');
  } catch (error) {
