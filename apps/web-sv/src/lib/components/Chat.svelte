...
<div class="flex h-full flex-col bg-white dark:bg-gray-900">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="relative">
        ...
        {#if showModelSelector}
          <div class="absolute right-0 top-full z-10 mt-2 min-w-[200px] rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-800">
            {#each availableModels as model}
              <button
                onclick={() => handleModelChange(model)}
                class="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100 dark:hover:bg-gray-700 {selectedModel === model ? 'bg-primary-50 text-primary-600 dark:bg-primary-900/20 dark:text-primary-400' : 'text-gray-700 dark:text-gray-200'}"
              >
                {model}
              </button>
            {/each}
  ...
  >
    {#if messages.length === 0}
      <div class="flex h-full items-center justify-center text-gray-500 dark:text-gray-400">
        <div class="text-center">
          ...
          <p class="mt-1 text-sm">Ask me anything about your bundle analysis or code optimization</p>
        </div>
      </div>
    {:else}
      <div class="space-y-4">
    ...
    {#if error}
      <div class="mt-4 rounded-lg border border-red-200 bg-red-50 p-4 dark:border-red-900 dark:bg-red-950/20">
  ...
  <div class="border-t border-gray-200 bg-gray-50 px-4 py-4 dark:border-gray-700 dark:bg-gray-800">
    <form onsubmit={handleSubmit} class="flex gap-2">
      <input
        ...
        disabled={isLoading}
        class="flex-1 rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 placeholder-gray-400 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500 disabled:opacity-50 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-500"
      />
      <button
        type="submit"
