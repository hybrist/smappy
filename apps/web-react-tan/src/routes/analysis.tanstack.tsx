/**
 * Analysis Detail Route - TanStack Start Version
 * Demonstrates parallel server function calls with TanStack Start
 */
...
// Define the route with loader using server functions directly
export const analysisRoute = createRoute({
  getParentRoute: () => rootRoute,
  path: "/analysis/$analysisId",
  loader: async ({ params }) => {
    // Call server functions directly - parallel execution!
    const [analysis, bundles, modules] = await Promise.all([
      getAnalysisDetails(params.analysisId),
      getAnalysisBundles(params.analysisId),
      getAnalysisModules(params.analysisId, {
        page: 1,
        pageSize: 20,
        sortBy: "bundledSize",
        sortOrder: "desc",
