...
export const getProjectAnalyses = createServerFn(
  "GET",
  async (projectName: string): Promise<AnalysisRun[]> => {
    const runs = await db
      .select({
        id: schema.analysisRun.id,
...
export const getAnalysisDetails = createServerFn(
  "GET",
  async (id: string): Promise<AnalysisRun | null> => {
    const analysisId = parseInt(id, 10);
    if (isNaN(analysisId)) {
      return null;
    }

    return getAnalysisByIdInternal(analysisId);
  },
);
...
export const getAnalysisBundles = createServerFn(
  "GET",
  async (id: string): Promise<Bundle[]> => {
    const analysisId = parseInt(id, 10);
    if (isNaN(analysisId)) {
      return [];
    }
...
export const getAnalysisDependencyGraph = createServerFn(
  "GET",
  async (id: string): Promise<DependencyNode[]> => {
    const analysisId = parseInt(id, 10);
    if (isNaN(analysisId)) {
      return [];
    }
...
export const getAnalysisTreemap = createServerFn(
  "GET",
  async (id: string): Promise<TreemapNode> => {
    const analysisId = parseInt(id, 10);
    if (isNaN(analysisId)) {
      return {
        name: "root",
