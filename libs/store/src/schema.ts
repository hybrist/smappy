...
export const module = sqliteTable(
  "Module",
  {
    ...
    isThirdParty: integer("is_third_party", { mode: "boolean" })
      .notNull()
      .default(false),
  ...
  (table) => ({
    ...
    pathRunUnique: unique("idx_module_path_run").on(
      table.analysisRunId,
      table.filePath,
    ),
  }),
...
export const dependency = sqliteTable(
  ...
  (table) => ({
    analysisRunIdx: index("idx_dependency_analysis_run").on(
      table.analysisRunId,
    ),
...
export const suggestion = sqliteTable(
  ...
  (table) => ({
    analysisRunIdx: index("idx_suggestion_analysis_run").on(
      table.analysisRunId,
    ),
  }),
);
...
export const suggestionLink = sqliteTable(
  ...
  (table) => ({
    pk: primaryKey({
      columns: [table.suggestionId, table.entityType, table.entityId],
    }),
    entityIdx: index("idx_suggestion_link_entity").on(
      table.entityType,
      table.entityId,
    ),
  }),
