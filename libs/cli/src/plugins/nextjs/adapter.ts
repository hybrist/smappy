...
export class NextJsAdapter extends WebpackAdapter {
  ...
  constructor(
    ...
    this.runtime =
      options.runtime ??
      (options.buildTarget === "edge" || options.buildTarget === "middleware"
        ? "edge"
        : "nodejs");
  }
  ...
  extract(bundlerOutput: unknown): PluginExtractionResult {
    ...
    return {
      ...
      modules: result.modules.map((module) => ({
        ...module,
        filePath: toAbsoluteModuleId(
          sanitizeModuleIdentifier(module.filePath),
          this.baseDir,
        ),
      })),
 ...
 */
function sanitizeAssetPath(
  fileName: string,
  target: NextJsBuildTarget,
): string {
  if (!fileName) {
    return fileName;
  }
 ...
 */
function splitResourceQuery(resource: string): {
  resourcePath: string;
  queryString?: string;
} {
