...
export class AngularAdapter extends BundlerAdapter {
  ...
  extract(bundlerOutput: unknown): PluginExtractionResult {
    ...
    try {
      ...
      const statsData =
        stats || (loadStats ? this.loadStatsJson(outputPath, errors) : null);
      ...
      if (statsData) {
        ...
        bundlerBundles = this.extractBundlesFromStats(
          statsData,
          outputPath,
          errors,
        );
        ...
        warnings.push(
          "No stats.json found, extracting from output directory only",
        );
   ...
   */
  private loadStatsJson(
    outputPath: string,
    _errors: string[],
  ): AngularStatsJson | null {
    try {
  ...
  private extractChunksFromStats(
    ...
    for (const chunk of stats.chunks) {
      ...
      const moduleIds = chunk.modules
        ? chunk.modules.map((m) => m.id || m.identifier)
        : [];
  ...
  private extractBundlesFromStats(
    ...
    for (const asset of stats.assets) {
      ...
      try {
        ...
        const sourceMap = this.extractSourceMapForBundle(
          bundlePath,
          outputPath,
        );
   ...
   */
  private extractBundlesFromDirectory(
    outputPath: string,
    errors: string[],
  ): BundlerBundle[] {
    const bundles: BundlerBundle[] = [];
   ...
   */
  private extractSourceMapForBundle(
    bundlePath: string,
    _outputPath: string,
  ): string | null {
