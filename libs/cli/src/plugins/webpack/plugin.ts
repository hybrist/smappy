      ...
      async (stats: Stats, callback: () => void) => {
        ...
        try {
          ...
          if (extractionResult.warnings.length > 0) {
            console.warn(
              "[Webpack Bundle Analysis] Warnings:",
              extractionResult.warnings,
            );
          }
          if (extractionResult.errors.length > 0) {
            console.error(
              "[Webpack Bundle Analysis] Errors:",
          ...
          if (this.options.autoIngest !== false) {
            ...
            try {
              ...
              console.error(
                "[Webpack Bundle Analysis] Failed to ingest bundles:",
                errorMessage,
              );
          ...
          const errorMessage =
            error instanceof Error ? error.message : String(error);
          console.error(
            "[Webpack Bundle Analysis] Failed to analyze bundle:",
            errorMessage,
          );
          if (this.config?.debug) {
            console.error("Full error details:", error);
